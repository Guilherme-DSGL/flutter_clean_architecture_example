...
void main() {
  ...
  group("get Concrete Number trivia tests", () {
    const tNumber = 1;
    const tNumberTriviaDTO = NumberTriviaDTO(text: "test", number: tNumber);
    const NumberTriviaEntity tNumberTriviaEntity = tNumberTriviaDTO;
    ...
    group("device is online", () {
      ...
      test(
          "should cahe the data locally when the call to remote data source is suscessful",
          () async {
        when(mockGetConcreteNumberTriviaDataSource(number: anyNamed("number")))
            .thenAnswer((_) async => tNumberTriviaDTO);
    ...
    group("device is offline", () {
      ...
      test(
          'should return last locally cache data when the cahe data is present',
          () async {
        ...
        expect(result, equals(const Right(tNumberTriviaDTO)));
      });
